...
const handleGetUserDashboard = async (
    ...
    try {
        if (userId) {
            ...
            // 1. Shiksha
            const { data: allCourses } = await getAllEnrolledCoursesFromDB(
                userId
            )
            if (allCourses) enrolledCourses = allCourses

            // 2. Projects
            const { data: allProjects } = await getAllEnrolledProjectsFromDB(
                userId
            )
            if (allProjects) enrolledProjects = allProjects

            // 3. Interview Sheets
            const { data: allUserSheets } = await getAllEnrolledSheetsFromDB(
                userId
            )
