...
import type { APIResponseType, BuildOrderPayloadProps, PlaylistModel, UserPointsActionType, Video, WebhookEvent } from "@/interfaces"

import { envConfig, JOB_SKILL_NORMALIZER, SKILL_BLACKLIST, YOUTUBE_API_PATH } from "@/config/constants"
...
const fetchAPIData = async (url: string): Promise<{ status: boolean; data: any }> => {
...
const fetchPlaylistName = async (
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch playlist metadata: ${data.error?.message || 'Unknown error'}`
            )
        }
...
const fetchPlaylistData = async (
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch playlist data: ${data.error?.message || 'Unknown error'}`
            )
        }
